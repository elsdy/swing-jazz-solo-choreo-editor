...
**서버의 살림은 서버가 정한다(2026-09-13).** 보관 루트·하위 폴더·모델 폴더·LLM 설정은 서버의 것이고, 바꾸는 자리는 서버가 스스로 내는 관리 화면 `/admin`(저장소 루트의 `admin.html`) 하나다. 안무 편집기는 그 값을 `GET /api/config` 로 **읽기만** 한다 — 클라이언트가 서버의 디스크를 정하면 브라우저마다 다른 답을 들고 같은 서버를 서로 다르게 설정하게 되기 때문이다(폰과 PC 가 실제로 갈렸다). ⚠ `admin.html` 은 `src/` 를 import 하지 않는다. 앱의 모듈이 아니라 서버의 화면이라, 앱이 깨져도 떠야 한다.
