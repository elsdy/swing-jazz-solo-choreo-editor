...
브라우저에서 도는 정적 파일이 앱의 전부이고, 안무는 JSON 파일로 저장해 주고받는다. 그 옆에 영상과 안무표를 **저장소 밖의 데이터 폴더**에 보관해 주는 로컬 서버 `server.py` 가 하나 있다(파이썬 표준 라이브러리만 쓴다). 서버 없이 정적 호스팅으로 열어도 앱은 그대로 동작한다.
...
그리고 브라우저에서 `http://localhost:8000` 을 연다. 영상 패널에서 고른 영상 파일은 데이터 폴더의 `video-clip/<프로젝트 이름>/` 아래에 복사되고, 안무표를 다시 열면 자동으로 돌아온다. 그 자리가 어디인지는 서버가 켜질 때 첫 줄에 찍는다(macOS 는 `~/Library/Application Support/choreo`, 리눅스는 `~/.local/share/choreo`, `XDG_DATA_HOME` 을 따른다). 저장소 안이 아니라 밖인 까닭은 안무표와 영상이 잃으면 복구 못 하는 것이고 저장소는 다시 받으면 그만이기 때문이다. 영상의 한 구간만 잘라 새 파일로 만드는 `✂ 잘라서 새 클립으로` 는 서버를 켠 컴퓨터에 `ffmpeg` 이 있을 때만 켜진다(`brew install ffmpeg`, 없어도 나머지는 다 된다). 보관 위치는 `python3 server.py --root <폴더>` 또는 앱의 `⚙ 설정` 에서 바꾼다. `python3 -m http.server 8000` 으로도 앱은 뜨지만 영상 보관은 브라우저 폴더 방식(크롬 계열)이 된다.
...
- **영상 맞춰 보기** — 붙여 둔 YouTube 영상을 안무표 옆에 띄우고, 박자를 두 번 찍어 맞추면 재생 위치가 안무표 위에 세로선으로 표시된다. 블록을 누르면 영상이 그 대목으로 건너뛴다(인터넷 필요).
