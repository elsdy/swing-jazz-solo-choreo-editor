...
브라우저에서 도는 정적 파일이 앱의 전부이고, 안무는 JSON 파일로 저장해 주고받는다. 그 옆에 업로드한 영상을 `video-clip/` 아래에 보관해 주는 로컬 서버 `server.py` 가 하나 있다(파이썬 표준 라이브러리만 쓴다). 서버 없이 정적 호스팅으로 열어도 앱은 그대로 동작한다.
