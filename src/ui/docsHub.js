...
// ``` 코드블록 · > 인용 · --- 구분선 · **굵게** · *기울임* · ~~취소선~~ · `코드` · [링크](대상)
...
function inline(md) {
  ...
  // ⚠ 취소선이 **굵게보다 먼저**다(2026-09-13). `~~` 안에 `**` 가 드는 일이 흔한데
  //   순서가 반대면 굵게 규칙이 먼저 먹어 물결표만 글자로 남는다. 이 저장소의 로드맵·PORTS 가
  //   「정했다」를 취소선으로 적어 온 터라, 없는 동안 여덟 줄이 `~~...~~` 그대로 보이고 있었다.
  s = s.replace(/~~([^~\n]+)~~/g, '<del>$1</del>');
